...
| [claude-code-reflect-nav-consolidation-spec.md](specs/claude-code-reflect-nav-consolidation-spec.md) | 【A】Reflect OS 導線整理（ホーム重点を閲覧専用化・テーマ未紐づけをログ内フィルタへ・検索をアーカイブに統合＝ナビ10→8）＋重点スナップショット reflect_focus_v1 をlocalStorageへミラー（Bの前提） | 🕒未実装 |
| [claude-code-taskos-focus-display-spec.md](specs/claude-code-taskos-focus-display-spec.md) | 【B・要A先行】Task OS HOMEに重点の問い/実験中を閲覧専用カード表示（reflect_focus_v1参照・空なら非表示）＋参謀提案のR系実験（Lecticaマスタ外）受け入れ | 🕒未実装 |
