...
| [claude-code-lectica-random-removal-and-full-memo-spec.md](specs/claude-code-lectica-random-removal-and-full-memo-spec.md) | Lecticaランダム生成を完全廃止・R系(マスタ外)受け入れ・done時も別ID提案は新規追加・Shotカードに実験内容フル表示・完了✔に確認ダイアログ・⑥pending新旧フィールド名両対応 | ①〜⑤=✅実装済み `194e881`／⑥=🕒未実装（改訂版で追加。現行はexperimentId/titleを読まずselectedExperimentId/Titleのみ＝Cowork新名で「(実験名なし)」になる実バグ・launcher5箇所+task-os2箇所） |
