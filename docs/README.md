...
| [claude-code-lectica-random-removal-and-full-memo-spec.md](specs/claude-code-lectica-random-removal-and-full-memo-spec.md) | Lecticaランダム生成を完全廃止・R系(マスタ外)受け入れ・done時も別ID提案は新規追加・Shotカードに実験内容フル表示・完了✔に確認ダイアログ・⑥pending新旧フィールド名両対応 | ①〜⑤=✅実装済み `194e881`／⑥=✅実装済み（field-fix-specで対応） |
| [claude-code-shot-pending-back-and-field-fix-spec.md](specs/claude-code-shot-pending-back-and-field-fix-spec.md) | ①Shot保留カードに「未着手に戻す」＋自動pause戻し時のフラグ掃除・期日引き上げ ②依頼書⑥の実装漏れ対応（aix_lectica_pending読取を新旧フィールド名両対応に） | ①=✅`a6b1498`／②=✅実装済み（launcher6箇所+task-os5箇所をexperimentId/title優先の両対応化） |
| [claude-code-taskos-routine-nextdate-unify-spec.md](specs/claude-code-taskos-routine-nextdate-unify-spec.md) | Task OS completeRt の次回日計算を Routine OS の calcNextRoutineDate に統一（加算基準・サイクル・キャッチアップ尊重。期日切れ後の完了でも曜日が守られる。計算不能時は上書きせず警告）。routine-osの日付計算群9関数を移植＋両OS同期コメント契約 | ✅実装済み 2026-08-23（10関数が本家と同一実装で一致・挙動9項目PASS） |
| [claude-code-routine-addbase-dispatch-spec.md](specs/claude-code-routine-addbase-dispatch-spec.md) | 【追補v2】ルーチン完了時の次回日計算を加算基準(addBase)でディスパッチ。from_ms/from_me/from_ys/fixed/from_lastはcalcNext（営業日・例外加算日数・祝日/週末除外）＋キャッチアップ、from_nextは現行サイクル計算を維持。doComplete/completeRt両方＋calcNext依存群をtask-osへ移植（同期契約） | ✅実装済み 2026-08-23（PL実績8/22→9/7・経営計画8/21→9/8・祝日除外・キャッチアップ・from_nextリグレッション検証済み。移植10関数が本家と同一実装で一致。doSkipは対象外） |
| [claude-code-tankyu-board-r-link-spec.md](specs/claude-code-tankyu-board-r-link-spec.md) | 探求ボードのR系連動（追補J）：実験にextId（R系ID）を導入しLectica取り込み（実験ID/関連実験）と連動して実践回数・最終実践日・実験/親の問いの更新日を自動更新＋[R系ID]バッジ表示。task-osバックアップのReflect収集にquestions/experiments/principlesを追加 | ✅実装済み 2026-08-29（reflect-os=`56d17d6`マージ適用〈添付は7e1585d前土台のため無改変配置せず新機能のみ適用〉・R系連動13件PASS／task-os=2行差分・dump/restore7件PASS・旧バックアップ互換確認） |
| [taskos-shot-not-created不具合報告書.md](specs/taskos-shot-not-created不具合報告書.md) | GDrive同期してもLectica Shotが生成されない（8/21実機・BUILD_VER 2026-08-18時点）。核心=当日Shot 0件時の新規生成パス欠落 | ✅`194e881`で解消（新規生成パスを実装）。現行コードで受け入れ4点＋複数行memo＋マスタ外IDを再現テスト全PASS。読取は単一の gdFindDailyFile('daily-tasks','aix-tasks')経由でpending書込1箇所＝追補の「直下別経路」は現行に該当せず |
| [reflect_card_K_0918.md](specs/reflect_card_K_0918.md) | Reflect OS改修カード追補K（パッチAI対応）：K-1=取り込みの関連実験行ID抽出寛容化＋「再演」をrelatedNoteとして保存／K-2=L主＋L関連の受け入れ確認／K-3=壁打ちプロンプト定数(SPARRING)をv3・2026-09-18版へ差し替え | K-1=✅`a4e1c75`（ID寛容化は既存relRaw正規表現で充足・relatedNote保存を追加）／K-2=✅受け入れ確認済み／K-3=✅実装済み 2026-09-19（launcher/reflect-osのLX_PROMPT_SPARRINGを正本v3.1〈2026-09-19・質問数上限なし〉全文へ差し替え・両定数一致・docs/prompts正本もv3.1に更新・差分確認書A〜E全点PASS） |
